...
from wyrdbound_model.core.schema import ModelDefinition, AttributeDefinition, ValidationRule
...
class TestWyrdboundModel:
    ...
    def test_model_with_inheritance_resolve_inheritance(self):
        ...
        # Mock the inheritance resolver to provide the base model
        from unittest.mock import Mock
        mock_resolver = Mock()
        mock_resolver.resolve_inheritance.return_value = base_model_def
        
        # Test successful inheritance
        model_registry = {
        ...
        mock_resolver.resolve_inheritance.side_effect = Exception("Inheritance resolution failed")
        
        with pytest.raises(InheritanceError):
            WyrdboundModel(
                child_model_def,
                {"name": "Failed"},
                inheritance_resolver=mock_resolver
            )
