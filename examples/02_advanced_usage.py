...
def main():
    ...
    character_def = ModelDefinition(
        ...
        attributes={
            ...
            "stat_total": AttributeDefinition(
                type="int", derived="{{ strength + dexterity + intelligence }}"
            ),
            "health_bonus": AttributeDefinition(
                type="int", derived="{{ (level - 1) * 5 + strength * 2 }}"
            ),
            "max_health": AttributeDefinition(
                type="int", derived="{{ health + health_bonus }}"
            ),
            "mana_bonus": AttributeDefinition(
                type="int", derived="{{ (level - 1) * 3 + intelligence * 3 }}"
            ),
            "max_mana": AttributeDefinition(
                type="int", derived="{{ mana + mana_bonus }}"
            ),
            "combat_rating": AttributeDefinition(
                type="int", derived="{{ (strength * 2 + dexterity + level) // 2 }}"
            ),
        ...
        validations=[
            ValidationRule(
                expression="level >= 1", message="Character level must be at least 1"
            ),
            ValidationRule(
                expression="experience >= 0", message="Experience cannot be negative"
            ),
            ValidationRule(
                expression="stat_total >= 30", message="Total stats must be at least 30"
            ),
        ],
    )
    ...
    print(
        f"  Stats: STR {character['strength']}, DEX {character['dexterity']}, INT {character['intelligence']}"
    )
    ...
    try:
        ...
        print(
            f"✗ Invalid character should not have been created: {invalid_character['name']}"
        )
    except Exception as e:
    ...
    character["equipment"] = {
        "weapon": {"name": "Orcrist", "damage": 15, "type": "sword"},
        "armor": {"name": "Mithril Shirt", "defense": 20, "type": "chainmail"},
        "accessories": [
    ...
    item_def = ModelDefinition(
        ...
        attributes={
            "weight": AttributeDefinition(
                type="float", default=1.0, range="0.1..100.0"
            ),
        ...
        validations=[
            ...
            ValidationRule(
                expression="value > 0", message="Item value must be positive"
            ),
        ],
    )
    ...
    enhanced_weapon_def = ModelDefinition(
        ...
        attributes={
            "damage": AttributeDefinition(type="int", default=1, range="1..50"),
            "weapon_type": AttributeDefinition(type="str", default="sword"),
            "attack_speed": AttributeDefinition(
                type="float", default=1.0, range="0.5..3.0"
            ),
            ...
            "dps": AttributeDefinition(
                type="float", derived="{{ damage * attack_speed }}"
            ),
            "damage_rating": AttributeDefinition(
                type="str",
        ...
        validations=[
            ...
            ValidationRule(
                expression="attack_speed > 0", message="Attack speed must be positive"
            ),
        ],
    )
    ...
    print(
        f"  Item value: {excalibur['value']} gold, weight: {excalibur['weight']} lbs (from item)"
    )
    ...
    try:
        ...
        print(
            f"  ✓ Item validation caught negative weight: {str(e).split('|')[1].split(']')[0]}]"
        )
    ...
    try:
        ...
        print(
            f"  ✓ Weapon validation caught negative damage: {str(e).split('|')[1].split(']')[0]}]"
        )
