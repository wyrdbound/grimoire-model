...
def benchmark_operation(name: str, operation, iterations: int = 1000):
    ...
    print(
        f"{name}: {total_time:.4f}s total, {avg_time:.6f}s avg ({iterations} iterations)"
    )
    return avg_time
    ...
    return ModelDefinition(
        ...
        attributes={
            ...
            "health": AttributeDefinition(
                type="int", derived="{{ (constitution * 5) + (level * 10) }}"
            ),
            "mana": AttributeDefinition(
                type="int", derived="{{ (intelligence * 3) + (level * 5) }}"
            ),
            ...
            "attack_bonus": AttributeDefinition(
                type="int", derived="{{ (strength // 2) + (level // 4) }}"
            ),
            "defense_bonus": AttributeDefinition(
                type="int", derived="{{ (dexterity // 2) + (constitution // 3) }}"
            ),
            "spell_power": AttributeDefinition(
                type="int", derived="{{ (intelligence * 2) + (level // 2) }}"
            ),
            # Skill checks (derived)
            "physical_skills": AttributeDefinition(
                type="int", derived="{{ strength + dexterity + constitution }}"
            ),
            "mental_skills": AttributeDefinition(
                type="int", derived="{{ intelligence + wisdom + charisma }}"
            ),
            ...
            "total_power": AttributeDefinition(
                type="int", derived="{{ combat_rating + spell_power + (mana // 5) }}"
            ),
            # Character summary
...
def main():
    ...
    def batch_update_batched():
        ...
        batched_character.batch_update({
            "level": new_level,
            "strength": batched_character["strength"] + 1
            if batched_character["strength"] < 20
            else 12,
        })
    ...
    print(
        f"Active derived fields: {len(character._derived_field_resolver.derived_fields)}"
    )
    ...
    print(
        f"Round-trip successful: {character['character_summary'] == restored_character['character_summary']}"
    )
    print()
    ...
    def create_character_by_class(
        name: str, character_class: str, level: int = 1
    ) -> GrimoireModel:
        ...
        char_data.update({"name": name, "level": level})
    ...
    for character in party:
        ...
        print(
            f"    Combat Rating: {character['combat_rating']}, Total Power: {character['total_power']}"
        )
    print()
