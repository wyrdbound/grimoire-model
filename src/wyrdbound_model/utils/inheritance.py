...
def resolve_model_inheritance(
    ...
    inheritance_chain = _get_inheritance_chain(model_def, model_registry, max_depth)
    logger.debug(f"Inheritance chain for '{model_def.id}': {inheritance_chain}")
    
    # Resolve attributes from inheritance chain
    resolved_attributes = _resolve_attributes(inheritance_chain, model_registry)
    
    # Resolve validation rules
    resolved_validations = _resolve_validations(inheritance_chain, model_registry)
...
def _get_inheritance_chain(
    ...
    while queue and depth < max_depth:
        ...
        for parent_id in parent_ids:
            ...
            parent_model = model_registry[parent_id]
